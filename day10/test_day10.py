import unittest

import day10 as day


class Test(unittest.TestCase):

    def setUp(self):
        self.lines = [
            "7-F7-",
            ".FJ|7",
            "SJLL7",
            "|F--J",
            "LJ.LJ"
        ]

    def test_solve_part1_square_loop(self):
        lines = [
            ".....",
            ".S-7.",
            ".|.|.",
            ".L-J.",
            "....."
        ]
        self.assertEqual(4, day.solve_part1(lines))

    def test_solve_part1_complex_loop(self):
        lines = [
            "..F7.",
            ".FJ|.",
            "SJ.L7",
            "|F--J",
            "LJ..."
        ]
        self.assertEqual(8, day.solve_part1(lines))

    def test_solve_part2_small_example(self):
        lines = [
            "...........",
            ".S-------7.",
            ".|F-----7|.",
            ".||.....||.",
            ".||.....||.",
            ".|L-7.F-J|.",
            ".|..|.|..|.",
            ".L--J.L--J.",
            "..........."
        ]
        self.assertEqual(4, day.solve_part2(lines))

    def test_solve_part2_large_example(self):
        lines = [
            ".F----7F7F7F7F-7....",
            ".|F--7||||||||FJ....",
            ".||.FJ||||||||L7....",
            "FJL7L7LJLJ||LJ.L-7..",
            "L--J.L7...LJS7F-7L7.",
            "....F-J..F7FJ|L7L7L7",
            "....L7.F7||L7|.L7L7|",
            ".....|FJLJ|FJ|F7|.LJ",
            "....FJL-7.||.||||...",
            "....L---J.LJ.LJLJ..."
        ]
        self.assertEqual(8, day.solve_part2(lines))

    def test_solve_part2_extra_pipes(self):
        lines = [
            "FF7FSF7F7F7F7F7F---7",
            "L|LJ||||||||||||F--J",
            "FL-7LJLJ||||||LJL-77",
            "F--JF--7||LJLJ7F7FJ-",
            "L---JF-JLJ.||-FJLJJ7",
            "|F|F-JF---7F7-L7L|7|",
            "|FFJF7L7F-JF7|JL---7",
            "7-L-JL7||F7|L7F-7F7|",
            "L.L7LFJ|||||FJL7||LJ",
            "L7JLJL-JLJLJL--JLJ.L"
        ]
        self.assertEqual(10, day.solve_part2(lines))


if __name__ == '__main__':
    unittest.main()
